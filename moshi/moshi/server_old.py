...
#
#   7. FIX: RAG injection is now triggered ONLY by the model's own <ref>
#      token (matching how train_rag.py's inject_streaming_sum_text_channel
#      only adds the conditioning vector from meta.body_start onward — i.e.
#      strictly after <ref> + filler). Previously the code also fired the
#      injection pipeline immediately on VAD end-of-utterance, well before
#      the model had even started speaking. That activates the conditioning
#      vector during a time window the model was never trained to see it
#      in, which is why context was being ignored. VAD now only captures
#      the transcript into current_user_text and resets STT; the <ref>
#      token remains the sole injection trigger.
#   8. FIX: explicit `listening` / `generating` gating — STT only steps
#      while `listening and not generating`, so it never runs during the
#      assistant's own turn, and always gets a clean per-utterance reset.
...
class ReferenceAdapter(nn.Module):
    ...
    NOTE ON MEAN POOLING: do NOT remove this. train_rag.py's ReferenceAdapter
    (and inject_streaming_sum_text_channel) were trained to add ONE pooled
    [dim] vector, broadcast identically across every body frame:
        text_emb[b, body_start:] = text_emb[b, body_start:] + streaming_sum[b]
    fc1/fc2 are Linear(dim, ...) layers that expect a single pooled [dim]
    vector, not a [T, dim] sequence — the checkpoint's weights were never
    trained to consume per-token reference embeddings. Skipping the mean
    pool would feed the trained fc1/fc2 weights an input shape/distribution
    they never saw, which would make grounding worse, not better.
    """
...
class ServerState:
    ...
    async def run_rag_injection(
        ...
        IMPORTANT: this must only ever be called from the <ref> token
        detection branch in opus_loop, never from VAD. train_rag.py's
        inject_streaming_sum_text_channel only adds the conditioning
        vector from meta.body_start onward — i.e. strictly after the
        model has emitted <ref> and worked through its filler segment.
        Activating the injector any earlier (e.g. right at VAD end,
        before the model has even started responding) puts the vector
        in front of the model during a time window it was never trained
        to see it in, which silently defeats grounding.
        """
        t0 = time.monotonic()
        ...
        def _encode_ref():
            ...
            with torch.no_grad():
                vec = self.adapter(id_tensor)  # [dim] — mean-pooled, matches training
            return vec
    ...
    async def handle_chat(self, request):
        ...
        # listening / generating are kept as two explicit, mutually
        # exclusive gates:
        #   listening  — True only while we're waiting on / capturing the
        #                user's next query. STT only ever steps when
        #                listening is True AND generating is False.
        #   generating — True only while the assistant's own response is
        #                being produced (mirrors assistant_is_speaking).
        # Both flip together at each transition so there's never a window
        # where STT runs during the assistant's turn, or vice versa.
        listening:  bool = True
        ...
        def _on_response_end():
            ...
            # Back to idle — fully reset STT so the next utterance starts
            # from a clean streaming state instead of dragging the whole
            # session's context along with it, and start listening again.
            if self.stt_lm_gen is not None:
                self.stt_lm_gen.reset_streaming()
                self.stt_mimi.reset_streaming()
            stt_token_buffer.clear()
        ...
        async def _trigger_rag_injection(question: str):
            ...
            Called ONLY from the <ref> token detection branch below — never
            from VAD. See the note on run_rag_injection for why: the
            conditioning vector must only turn on once the model itself
            has reached the point in its own generation where it was
                ...
                while True:
                    ...
                    while all_pcm_data.shape[-1] >= self.frame_size:
                        ...
                        def _step():
                            ...
                            # STT only steps while we're actually listening
                            # for the user's next query, never during the
                        ...
                        # ── STT / VAD ─────────────────────────────────────
                        # NOTE: this block only ever CAPTURES the transcript
                        # into current_user_text and stops listening. It
                        # does NOT trigger the RAG pipeline — that happens
                        # solely from the <ref> token branch below, once the
                        # model itself decides to ground its answer.
                        if stt_result is not None and listening and not generating:
                            ...
                            if stt_tokens is not None:
                                ...
                                text_token_stt = stt_tokens[0, 0, 0].item()
                                if text_token_stt not in (0, 3):
                                    stt_in_utterance = True
                                    stt_last_vad_end = False

                            # Silence when VAD score is above threshold (high = silence)
                            ...
                            if vad_fired and stt_in_utterance and stt_token_buffer:
                                ...
                                if transcript.strip():
                                    ...
                                    clog.log("info", f"[STT] query: {current_user_text!r}")

                                # We only need this turn's query, not a running
                                # transcript — stop listening and reset the STT
                                # model's streaming state right away so it
                                # starts clean on the next utterance instead of
                                # carrying the whole session forward. RAG is
                                # deliberately NOT triggered here — see the
                                # <ref> token branch below.
                                listening = False
                                if self.stt_lm_gen is not None:
                                    self.stt_lm_gen.reset_streaming()
                                    self.stt_mimi.reset_streaming()
                        ...
                        # ── <ref> token detection ─────────────────────────
                        # This is the ONLY place the RAG pipeline is
                        # triggered, matching how train_rag.py only adds
                        # the conditioning vector from body_start onward
                        # (i.e. strictly after <ref> + filler).
                        if (
                            self.rag_enabled
                            ...
                            if not assistant_is_speaking:
                                ...
                                generating = True
                                # Assistant started talking (possibly before
                                # STT ever captured a query, e.g. model-
                                # initiated speech) — stop listening and
                                # reset STT so it doesn't keep running
                                # through the assistant's own turn.
                                listening = False
                                if self.stt_lm_gen is not None:
                                    self.stt_lm_gen.reset_streaming()
                                    self.stt_mimi.reset_streaming()
