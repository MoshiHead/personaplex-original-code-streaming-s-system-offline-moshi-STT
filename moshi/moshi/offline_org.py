...
class ContextCompressor:
    ...
    # )
    COMPRESS_PROMPT = (
    "Answer using the passages and recent conversation. Infer only if supported. "
    "\n\n"
    "History:\n{history}\n\n"
    "Question:\n{question}\n\n"
    "Passages:\n{passages}\n\n"
...
class ServerState:
    ...
    async def handle_chat(self, request):
        ...
        async def start_rag_turn(transcript: str):
            ...
            ids = self.text_tokenizer.encode(ref_content.strip())
            if len(ids) > self.max_ref_tokens:
                ids = ids[: self.max_ref_tokens]
                ref_content = self.text_tokenizer.decode(ids)
            ref_tokens = self.text_tokenizer.encode(wrap_with_ref_tags(ref_content))
