...
class ServerState:
    ...
    async def handle_chat(self, request):
        ...
        # STT only runs while we're actually waiting on the user's next
        # query. It goes False the moment we've captured this turn's
        # transcript (or the assistant starts talking) and comes back True
        # once we return to idle — so the STT model's own streaming state
        # never has to span an entire session, just one utterance at a time.
        listening: bool = True
                ...
                while True:
                    ...
                    while all_pcm_data.shape[-1] >= self.frame_size:
                        ...
                        def _step():
                            ...
                            stt_res = None
                            if self.rag_enabled and self.stt_lm_gen is not None and listening:
                        ...
                        # ── STT / VAD ─────────────────────────────────────
                        if stt_result is not None and listening:
                            ...
                            # Silence counting (VAD score > threshold means silence)
                            ...
                            if vad_fired and stt_in_utterance and stt_token_buffer:
                                ...
                                # Reset STT buffer and state immediately (before processing)
                                stt_token_buffer.clear()
                                stt_in_utterance = False
                                stt_silence_frame_count = 0
                                stt_last_vad_end = False
                                ...
                                if transcript.strip():
                                    ...
                                    # Switch to generation mode and stop listening
                                    listening = False
                                    # Reset STT model for next utterance (clean slate)
                                    if self.stt_lm_gen is not None:
                                        self.stt_lm_gen.reset_streaming()
                                        self.stt_mimi.reset_streaming()
                        
                                    # Trigger the RAG injection (original call)
